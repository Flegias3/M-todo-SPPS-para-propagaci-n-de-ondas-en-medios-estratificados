...
%% Variables a considerar para obtener las soluciones %Frecuencia propuesta
w0 = 83E6; %Frecuencia de operación
...
plot(alfa, EcDisp)
hold on
stem(NMO, ones(1,length(Eigenvalores(1,:))))
% plot(alfa.^2, ppval(EcDispSpline,alfa.^2),'c--')
% stem(NMO, ones(1,length(NMO)))
grid on
title('Ecuación de dispersión')
xlabel('\alpha (1/m)')
ylabel('\Lambda(\omega,\alpha)')
legend('\Lambda(\omega,\alpha)', '\alpha_{j}')